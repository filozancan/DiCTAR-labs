...
%% Problem 3 - Closed loop contrained QOC trough Receding Horizon Algorithm
...
figure(2)
stairs([0:sim_steps]*Ts, x_traj(2,:)), hold on, grid on, zoom on
xlabel('t(s)'), ylabel('x_2(t)')

sqrt(x_traj(1,:).^2+x_traj(2,:).^2)

figure(3)
stairs([0:sim_steps]*Ts, sqrt(x_traj(1,:).^2+x_traj(2,:).^2)), hold on, grid on, zoom on
yline(1e-4, ':r')
yline(-1e-4, ':r')
...
figure(4)
stairs([0:sim_steps-1]*Ts, u_traj'), hold on, grid on, zoom on
