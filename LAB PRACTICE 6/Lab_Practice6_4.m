...
figure(1)
stairs([0:1:sim_steps]*Ts, x_traj(1,:)), hold on, zoom on, grid on
xlabel('t(s)'), ylabel('x_1(t)')

figure(2)
stairs([0:1:sim_steps]*Ts, x_traj(2,:)), hold on, zoom on, grid on
xlabel('t(s)'), ylabel('x_2(t)')

figure(3)
stairs([0:1:sim_steps]*Ts, sqrt(x_traj(1,:).^2+x_traj(2,:).^2)), hold on, zoom on, grid on
yline(1e-4,':r')
yline(-1e-4,':r')
xlabel('t(s)'), ylabel('||x||_2(t)')

figure(4)
stairs([0:1:sim_steps-1]*Ts, u_traj), hold on, zoom on, grid on
