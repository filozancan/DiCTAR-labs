...
x_k=x0
x_traj(:,1)=x0;  %the state has 2 dimension
sim_steps=500
...
figure(1)
subplot(311)
stairs([0:1:sim_steps]*Ts,x_traj(1,:)), grid on, hold on
ylabel('x_1')
xlabel('t(s)')
subplot(312)
stairs([0:1:sim_steps]*Ts,x_traj(2,:)), grid on, hold on
ylabel('x_2')
xlabel('t(s)')
subplot(313)
stairs([0:1:sim_steps]*Ts,sqrt(x_traj(1,:).^2+x_traj(2,:).^2)), grid on, hold on
yline(1e-4,':r')
yline(-1e-4,':r')
ylabel('||x||_2')
xlabel('t(s)')
...
figure(2)
stairs([0:1:sim_steps-1]*Ts,u_traj), grid on, hold on
ylabel('u')
xlabel('t(s)')
...
%%
%Modifichiamo x_max mettendo 0.01 alla prima componente
%Introducing state cinstraints
...
xlabel('t(s)')
return
%We need an input in the opposite direction (and it's the optinum input!)
...
%In questo caso dà un errore!! La spiegazione sta nel caso di prima

%Una soluzione sta nell'aumentare la lunghezza delle predictions

%%
%Metto un horizon =10 posso mediare l'effort su più tempo
x_max=[0; 0.35]
Gx=B_cal;
hx=-A_cal*x0+repmat(x_max,3,1);   %given a matrix and replicate it according to a matrix with some dimension
...
%Cerca di capire come ha modificato il time horizon
